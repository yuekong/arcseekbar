...
/**
 * Created by chenj on 2020/9/28
 * Email 455584337@qq.com
 */
public class ArcPadSeekBar extends View {
    ...
    private static final int DEFAULT_BORDER_COLOR = 0xccf57c00;     // 默认描边颜色

    private static final int DEFAULT_THUMB_COLOR = 0xccf57c00;      // 拖动按钮颜色
    ...
    private static final int DEFAULT_THUMB_SHADOW_COLOR = 0xccf57c00; // 拖动按钮阴影颜色
    ...
    public ArcPadSeekBar(Context context) {
        this(context, null);
    }

    public ArcPadSeekBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ArcPadSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.ArcColorSeekBar);
        mArcColors = getArcColors(context, ta);
        mArcWidth = ta.getDimensionPixelSize(R.styleable.ArcColorSeekBar_arc_width, dp2px(DEFAULT_ARC_WIDTH));
        mOpenAngle = ta.getFloat(R.styleable.ArcColorSeekBar_arc_open_angle, DEFAULT_OPEN_ANGLE);
        mRotateAngle = ta.getFloat(R.styleable.ArcColorSeekBar_arc_rotate_angle, DEFAULT_ROTATE_ANGLE);
        mMaxValue = ta.getInt(R.styleable.ArcColorSeekBar_arc_max, DEFAULT_MAX_VALUE);
        mMinValue = ta.getInt(R.styleable.ArcColorSeekBar_arc_min, DEFAULT_MIN_VALUE);
        // 如果用户设置的最大值和最小值不合理，则直接按照默认进行处理
        if (mMaxValue <= mMinValue) {
            mMaxValue = DEFAULT_MAX_VALUE;
            mMinValue = DEFAULT_MIN_VALUE;
        }
        int progress = ta.getInt(R.styleable.ArcColorSeekBar_arc_progress, mMinValue);
        setProgress(progress);
        mBorderWidth = ta.getDimensionPixelSize(R.styleable.ArcColorSeekBar_arc_border_width, dp2px(DEFAULT_BORDER_WIDTH));
        mBorderColor = ta.getColor(R.styleable.ArcColorSeekBar_arc_border_color, DEFAULT_BORDER_COLOR);

        mThumbColor = ta.getColor(R.styleable.ArcColorSeekBar_arc_thumb_color, DEFAULT_THUMB_COLOR);
        mThumbRadius = ta.getDimensionPixelSize(R.styleable.ArcColorSeekBar_arc_thumb_radius, dp2px(DEFAULT_THUMB_RADIUS));
        mThumbShadowRadius = ta.getDimensionPixelSize(R.styleable.ArcColorSeekBar_arc_thumb_shadow_radius, dp2px(DEFAULT_THUMB_SHADOW_RADIUS));
        mThumbShadowColor = ta.getColor(R.styleable.ArcColorSeekBar_arc_thumb_shadow_color, DEFAULT_THUMB_SHADOW_COLOR);
        mThumbWidth = ta.getDimensionPixelSize(R.styleable.ArcColorSeekBar_arc_thumb_width, dp2px(DEFAULT_THUMB_WIDTH));
        mThumbMode = ta.getInt(R.styleable.ArcColorSeekBar_arc_thumb_mode, THUMB_MODE_STROKE);

        mShadowRadius = ta.getDimensionPixelSize(R.styleable.ArcColorSeekBar_arc_shadow_radius, dp2px(DEFAULT_SHADOW_RADIUS));
        ta.recycle();
    }
    ...
    private int[] getArcColors(Context context, TypedArray ta) {
        int[] ret;
        int resId = ta.getResourceId(R.styleable.ArcColorSeekBar_arc_colors, 0);
        if (0 == resId) {
            resId = R.array.arc_colors_trans_default;
        }
...
//        canvas.drawPath(mSeekPath, mArcPaint);
    ...
    private class OnClickListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onSingleTapUp(MotionEvent e) {
            ...
            // 事件回调
            if (null != mOnProgressChangeListener) {
                mOnProgressChangeListener.onProgressChanged(ArcPadSeekBar.this, getProgress(), true);
                mOnProgressChangeListener.onStopTrackingTouch(ArcPadSeekBar.this);
            }
            return true;
        }
    }
    ...
    public interface OnProgressChangeListener {
         ...
         */
        void onProgressChanged(ArcPadSeekBar seekBar, int progress, boolean isUser);
         ...
         * @param seekBar 拖动条
         */
        void onStartTrackingTouch(ArcPadSeekBar seekBar);
         ...
         * @param seekBar 拖动条
         */
        void onStopTrackingTouch(ArcPadSeekBar seekBar);
    }
    // endregion -----------------------------------------------------------------------------------
}