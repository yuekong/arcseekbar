...
public class ArcSeekBar extends View {
    ...
    public int[] mShaderColors2 = new int[]{0xFF313742, 0xFF313742, 0xFF313742, 0xFF313742, 0xFF313742};
    ...
    private float mLabelPercentageTextSize;

    /**
     * 字体颜色
     */
    private int mLabelTextColor = 0xFF333333;
    private int mLabelPercentageTextColor = 0xFF333333;
    ...
    private int mThumbColor2 = 0xFFFFFFFF;
    /**
     * 拖动按钮的半径
     */
    ...
    private boolean isStopDrag = true;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mLabelTextSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 40, displayMetrics);

        mLabelPercentageTextSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 20, displayMetrics);
        ...
        for (int i = 0; i < size; i++) {
            ...
            } else if (attr == R.styleable.ArcSeekBar_arcLabelPercentageTextColor) {
                mLabelPercentageTextColor = a.getColor(attr, mLabelTextColor);
            ...
            } else if (attr == R.styleable.ArcSeekBar_arcShowShadow) {
                isShader = a.getBoolean(attr, false);
            }
        }
    ...
    private void drawThumb(Canvas canvas) {
        if (isShowThumb) {
            ...
            if (isShader) {
                ...
                mPaint.setColor(mThumbColor2);
                canvas.drawCircle(mThumbCenterX, mThumbCenterY, mThumbRadius / 2 + 2 + mThumbRadiusEnlarges, mPaint);
//        canvas.drawText(mProgressPercent + "%",mThumbCenterX,mThumbCenterY,mTextPaint);
...
//        }
    ...
    private void drawText(Canvas canvas) {
        ...
        float fontHeight = fontMetrics.bottom - fontMetrics.top;
//    // 计算文字baseline 
//    float textBaseX = mThumbCenterX/2 + mLabelPaddingLeft - mLabelPaddingRight;
//    float textBaseY = mThumbCenterY/2  - (mThumbCenterY /2 - fontHeight) / 2 - fontMetrics.bottom + mLabelPaddingTop - mLabelPaddingBottom;
//    if(isShowPercentText){//是否显示百分比
//      canvas.drawText(mProgressPercent + "%",textBaseX,textBaseY,mTextPaint);
//    }else if(!TextUtils.isEmpty(mLabelText)){//显示自定义文本
//      canvas.drawText(mLabelText,textBaseX,textBaseY,mTextPaint);
//    }
        canvas.drawText(mProgressPercent + "", mThumbCenterX, mThumbCenterY + fontHeight / 3, mTextPaint);
    ...
    /**
     ...
     //   * @param color
     //   */
//  public void setLabelPercentageTextColor(int color) {
//    this.mLabelPercentageTextColor = color;
//    invalidate();
//  }
...
//    setLabelPercentageTextColor(color);
    }
    ...
    public interface OnChangeListener {
        ...
        void onProgressChanged(int progress, float max, boolean fromUser);
    ...
    // 根据 resId 获取颜色数组
    public int[] getColorsByArrayResId(Context context, int resId) {
        int[] ret;
    ...
    // 获取 Arc 颜色数组
    public int[] getArcColors(Context context) {
        int[] ret;
        int resId = R.array.arc_colors_default;
